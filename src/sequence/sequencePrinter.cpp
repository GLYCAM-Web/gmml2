    ...
    namespace sequence
    {
        namespace
        {
            ...
            auto iupacNames = [](const SequenceData& sequence)
            {
                ...
                for (size_t n = 0; n < residueCount; n++)
                {
                    ResidueType type = residues.type[n];
                    // Iupac doesn't write -OH, but does include reducing terminal when it's sugar-sugar ano-ano
                    if (type == ResidueType::Sugar)
                    {
                        result.push_back(residues.name[n] + residues.modifier[n]);
                    }
                    else if (util::contains({ResidueType::Deoxy, ResidueType::Derivative}, type))
                    {
                        result.push_back(
                            residues.preIsomerModifier[n] + residues.isomer[n] + residues.name[n] +
            ...
            { return edgesSortedByLink(sequence, edgeIds); };

            auto noLinkageBrackets = [](const SequenceData& sequence)
            { return std::vector<util::Brackets>(nodeCount(sequence.graph), util::noBrackets); };
            ...
            struct SequencePrintData
            {
                ...
                std::vector<ResidueType> residueTypes;
                std::vector<std::string> residueNames;
                std::vector<std::string> parentlessResidueLabel;
                std::vector<util::Brackets> residueLinkageBrackets;
                ...
                std::vector<std::string>& output)
            {
                ...
                if (!derivatives.empty())
                {
                    ...
                    outputResidueString += util::join(
                        data.derivativeSeparator, util::reverse(derivatives)); // order should be 2S,6S, not 6S,2S.
                ...
                outputResidueString +=
                    (hasParentEdge ? data.edgeNames[parentEdgeId] : data.parentlessResidueLabel[residueId]);
        ...
        std::string printSequence(const SequencePrintConfig& config, const SequenceData& sequence)
        {
            ...
            std::vector<std::string> residueNames =
                joinStrings(config.residueNames(sequence), config.residueLabels(sequence));
            std::vector<std::string> edgeNames = joinStrings(
                config.edgeNames(sequence, residueParentEdges), config.edgeLabels(sequence, residueParentEdges));
