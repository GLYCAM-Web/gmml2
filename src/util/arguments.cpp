...
namespace gmml
{
    namespace
    {
        std::string argHelpString(const util::ArgDef& def)
        {
            bool hasShortName = def.shortName != ' ';
            bool hasLongName = def.longName != "";
            std::string valueStr = (def.type == util::ArgType::option ? (" <" + def.nameOfValue + ">") : "");
    ...
    namespace util
    {
        ...
        Arguments readArguments(int argc, char* argv[], const std::vector<ArgDef>& defs)
        {
            ...
            for (size_t n = 0; n < arguments.size(); n++)
            {
                ...
                if (startsWith(arg, "--"))
                {
                    ...
                    auto hasName = [&](const ArgDef& def) { return def.longName == name; };
                    ...
                    switch (def.type)
                    {
                        ...
                        default:
                            throw std::runtime_error(
                                "erroneous argument type in " + arg + ". This is not a user error");
                    }
                }
                else if (startsWith(arg, "-"))
                {
                    ...
                    for (size_t k = 0; k < chars.size(); k++)
                    {
                        ...
                        auto hasName = [&](const ArgDef& def) { return def.shortName == c; };
                        auto it = std::find_if(defs.begin(), defs.end(), hasName);
                        if (it == defs.end())
                        {
                            throw std::runtime_error(
                                "unknown argument " + dashArg + (chars.size() > 1 ? (" in " + arg) : ""));
                        }
                        size_t index = it - defs.begin();
                        const ArgDef& def = defs[index];
                        switch (def.type)
                        {
                            ...
                            case ArgType::option:
                                {
                                    ...
                                    {
                                        throw std::runtime_error(
                                            "cannot group non-flag argument " + dashArg + " with -" + chars + "\ntry " +
                                            dashArg + " <" + def.nameOfValue + ">");
                                    }
                                    n++;
                                    if (n >= arguments.size())
                                    {
                            ...
                            default:
                                throw std::runtime_error(
                                    "erroneous argument type in " + dashArg + ". This is not a user error");
                        }
                    }
                }
                else
            ...
            auto isUnnamed = [](const ArgDef& def) { return def.type == ArgType::unnamed; };
        ...
        void validateRequiredArguments(const Arguments& arguments, const std::vector<ArgDef>& defs)
        {
            ...
            {
                if ((defs[n].type != ArgType::unnamed) && (defs[n].requirement == ArgReq::required) &&
                    !arguments.found[n])
                {
        ...
        void validateArgumentCount(const Arguments& arguments, const std::vector<ArgDef>& defs)
        {
            ...
            if (argCount < argMin || argCount > argMax)
            {
                ...
                throw std::runtime_error(
                    "expected " + numStr + " unnamed arguments, got " + std::to_string(arguments.unnamed.size()));
            }
        }
        ...
        std::string helpString(const std::string& programName, const std::vector<ArgDef>& defs)
        {
            ...
            auto braceType = [&](ArgReq req) { return (req == ArgReq::optional) ? brackets : emptyBrace; };
