...
namespace gmml
{
    namespace off
    {
        namespace
        {
            std::string residueOffType(const ResidueType queryType)
            {
                if (queryType == ResidueType::Protein)
                {
                    return "p";
                }
                else if (queryType == ResidueType::Solvent)
                {
                    return "w";
                }
                return "?";
                ...
                const std::string& unitName)
            {
                ...
                for (size_t graphIndex : residueIndices)
                {
                    ...
                    for (size_t residueAtom : assembly::residueAtoms(graph, graphIndex))
                    {
                        ...
                        util::writeFloat(stream, format.charge, atoms.charges[atomIndex]);
                        stream << "\n";
                        atomNumberInResidue++;
                    }
                }
                ...
                // WriteConnectSection
                //  Note: this is silly but fine for most cases. If you're reading this it's because it mattered and you
                ...
                // WriteResidueConnectSection // Every residue needs a head/tail regardless of reality. tleap uses this
                // info.
                stream << "!entry." << unitName
                ...
                for (size_t graphIndex : residueIndices)
                {
                    ...
                    { // Repeating the same atom changes the tree structure in the parm7 file. Not sure anything uses
                      // that. Old gmml code repeats so doing that.
                        // For reducing terminal old code puts a 1 2 0 0 0 0. So not repeat. Changing to 2 2 0 0 0 0
                        // causes first atom not to be M (main). Might be an issue let's see.
        ...
        { // For writing residues together as a molecule
            ...
            stream << " \"" << unitName << "\""
                   << "\n";
            writeUnit(stream, data.format, graph, data.residues, data.atoms, residueIndices, unitName);
        }
    } // namespace off
} // namespace gmml
