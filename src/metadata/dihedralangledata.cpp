...
namespace gmml
{
    namespace
    {
        ...
        //    additional angle StringVector residue1_conditions_ ; StringVector residue2_conditions_ ; std::string
        //    atom1_ ; std::string atom2_ ; std::string atom3_ ; std::string atom4_ ;
        //} ;
         ...
         * sugar, so would be 4 If two entries have matching regex and have the same index_ number (e.g. 1), the first
         * will be overwritten. If two entries have matching regex and different index_ numbers (e.g. 1,2,3) they will
         * all be used to create multiple rotamers/conformers If two entries have different regex, but apply to the same
         * dihedral angle (e.g. Phi), give them the same index_ number (e.g. 1).
         */
    ...
    std::vector<DihedralAngleData> dihedralAngleDataVector_ {
      { // Regex1  , Regex2   , Name   , Angle  , Deviation               , Weight, Entry Type               , Name , B , I , Res1 Condition , Res2 Conditions           , Atom names                                                               // Atom names this applies to
          ...
          { "C.*"  , "O6"     , "Omg"  ,  60.0  , AngleLimit{20.0, 20.0}  , 0.4   , RotamerType::permutation , "gt" , 3 , 2 , {}             , {"pyranose"}                  , "O6" , "C6" , "C5" , "O5"  },
...
//          { "C2"   , "O8"     , "Omg9" ,  71.1  ,   AngleStd{20.0, 20.0}  , 0.07  , RotamerType::conformer   , "E"  , 5 , 5 , {"ulosonate", "alpha", "external"}  , {"ulosonate"}    , "O9" , "C9" , "C8" , "O8"  },
  ...
  //        { "[SP]1", "O6"     , "Omg"  , 180.0  ,  20.0  ,  20.0  , 1.0   , RotamerType::permutation , "tg" , 3 , 3 , {"derivative"}       , {"gauche-effect=galacto"} , "O6" , "C6" , "C5" , "O5"  },
            ...
            const std::vector<std::string>& residue_types, const std::vector<std::string>& entry_conditions)
        {
            ...
            {
                if (!util::contains(residue_types, entry_condition))
                {
                    return false;
                }
            }
        ...
        std::string residueTypeToString(ResidueType resType)
        {
    ...
    std::vector<std::string> getTagsForResidue(const ResidueAttributes& residueAttributes)
    {
        std::string message = "Searching for attributes for " + residueAttributes.name;
        util::log(__LINE__, __FILE__, util::INF, message);
        ...
        std::vector<std::string> aldoseResidues = {
            "All", "Alt", "Ara", "Fuc", "Gal", "Glc", "Gul", "Ido", "Lyx", "Man", "Qui", "Rha",
            "Rib", "Tal", "Xyl", "Tyv", "dUA", "Bac", "Abe", "Oli", "AAT", "Mur", "man"}; // Note lowercase man
                                                                                          // as in LDmanHep :(
        ...
        if (util::contains(ketoseResidues, residueAttributes.name))
        {
            foundAttributes.push_back("ketose");
        }
        if (util::contains(ulosonateResidues, residueAttributes.name))
        {
        ...
        const ResidueAttributes& residue2Attributes)
    {
        ...
        for (size_t n = 0; n < table.entries.size(); n++)
        {
            ...
            {
                // Some entries have conditions for the residue, that they have certain tags. Make sure any conditions
                // are met:
                    ...
                    checkIfResidueConditionsAreSatisfied(residue2_types, entry.residue2_conditions_))
                {
                    ...
                    // Always add a later entry, but remove earlier match if number_of_bonds_from_anomeric_carbon_ AND
                    // index number are the same. I've overloaded the == and != operators in the DihedralAngleData
                    // struct to evaluate those.
        ...
        {
            maxMetadataDihedral =
                std::max(maxMetadataDihedral, table.entries[entry].number_of_bonds_from_anomeric_carbon_);
        }
