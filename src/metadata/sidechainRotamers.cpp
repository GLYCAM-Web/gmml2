...
namespace gmml
{
    ...
    {
        size_t residueId = util::indexOf(data.residues, residue);
        ...
        return util::indexVectorWithOffset(firstRotation, rotationCount);
    }

    SidechainRotamerData readSidechainRotamerData(const std::string& filename)
    {
        std::vector<char> buffer = util::readEntireFile(filename);
