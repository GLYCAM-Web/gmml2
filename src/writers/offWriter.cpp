...
namespace gmml
{
    ...
    void WriteOff(std::ostream& stream, const std::string& name, const GraphIndexData& data)
    {
        ...
        OffFileData offData = toOffFileData(data.objects.residues);
        ...
        WriteResiduesTogetherToOffFile(stream, graph, offData, util::indexVector(data.objects.residues), name);
    }
} // namespace gmml
