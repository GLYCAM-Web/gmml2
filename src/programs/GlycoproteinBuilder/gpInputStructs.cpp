    ...
    namespace gpbuilder
    {
        GlycoproteinBuilderInputs readGPInputFile(std::string inputFileName)
        {
            ...
            auto processLine = [&](const std::string& original, size_t lineNumber)
            {
                ...
                {
                    std::optional<ulong> opt = util::parseUlong(str);
                    if (!opt.has_value())
                    {
                ...
                {
                    std::optional<double> opt = util::parseDouble(str);
                    if (!opt.has_value())
                    {
                ...
                {
                    std::optional<bool> opt = util::parseBool(str);
                    if (!opt.has_value())
                    {
                ...
                const std::string line = util::trimmedOfWhitespace(original);
                if (!line.empty())
                {
            ...
            util::readFileLineByLine(inputFileName, processLine);
            if (!foundGlycanSection)
            {
                throw std::runtime_error(
                    "Error reading input file: '" + glycanSectionParameter + ":' section missing\n");
            }
            if (readingGlycanSection)
            {
                throw std::runtime_error(
            ...
            {
                if (!util::contains(foundParameters, req))
                {
