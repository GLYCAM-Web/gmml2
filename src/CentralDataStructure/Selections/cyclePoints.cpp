...
namespace gmml
{
    ...
    */
    std::vector<Atom*> FindCyclePoints(Atom* atom, Residue* residue)
    {
        ...
        std::vector<Atom*> rotation_points;
        std::vector<Atom*> atom_path;
        bool found = false;
        if (residue->GetType() == ResidueType::Protein)
        {
            Atom* caAtom = residue->FindAtom("CA");
            // Find any cycle points. Note: starting from connection point to other residue
            Atom* cycle_point;
            found = false;
            atom_path.clear();
            // This should only find cycles in Tyr, Trp etc. Not Asn, Ser, Thr as there aren't any unless bonding is
            // messed up.
    ...
    { // I definitely don't want cycles involving hydrogens (they only every form one bond, unless
        ...
        if (current_atom->getElement() != "H")
        {
            ...
            std::vector<Atom*> neighbors = current_atom->getNeighbors();
            for (std::vector<Atom*>::iterator it1 = neighbors.begin(); it1 != neighbors.end(); ++it1)
            {
                Atom* neighbor = *it1;
                ...
                {
                    if (util::contains(*atom_path, neighbor)) // If we've been at this atom before
                    {
    ...
    Atom* FindCyclePointNeighbor(const std::vector<Atom*> atom_path, Atom* cycle_point, Residue* cyclePointResidue)
    {
        ...
        if (cycle_point->getName() == "CA") // This is a protein, and we always want the N atom.
        {
          ...
          // fructose 1-1)
        else if ((cycle_point->getName() == "C2") && (!util::contains(atom_path, cyclePointResidue->FindAtom("C1"))))
        {
        ...
        else // This bit is overdone now, as I was looking for higher numbered atoms of C1, but now I know I always want
             // C2, so put that above.
        {
            std::vector<Atom*> neighbors = cycle_point->getNeighbors();
            // Ok must first get a list of neighbors that weren't in the connection path
            std::vector<Atom*> good_neighbors; // Couldn't think of a better name. Everybody needs these.
            for (auto& neighbor : neighbors)
            {
                if (!util::contains(atom_path, neighbor)) // If we've NOT been at this atom on way to cycle point
                {
                    if (neighbor->getName().at(0) != 'H') // Don't find hydrogens. Later we swap out to use a hydrogen
                                                          // to define a dihedral, but that's a very specific one.
                    {
            ...
            for (std::vector<Atom*>::iterator it1 = good_neighbors.begin(); it1 != good_neighbors.end(); ++it1)
            {
                Atom* neighbor = *it1;
                if (selected_neighbor->getName().size() >= 2)
                {
                    ...
                    } // Otherwise any neighbor is ok. Yes I'm comparing char's, but that is fine unless C9 Vs C10, but
                      // in that case I don't care again.
                }
            }
        }
