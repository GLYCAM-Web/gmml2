...
namespace gmml
{
        ...
        bool* found_neighbor)
    {
        ...
        for (auto& neighbor : current_atom->getNeighbors())
        {
            ...
            // If haven't visited this atom already AND don't move onto other residues
                ...
                (neighbor->getLabel() != "VisitedByFindAtomsConnectingResidues") &&
    ...
    std::vector<DihedralAtoms> splitAtomVectorIntoRotatableDihedrals(const std::vector<Atom*>& atoms)
    {
        ...
        {
            std::vector<DihedralAtoms> dihedrals;
            for (size_t n = 0; n < atoms.size() - 3; n++)
            {
                dihedrals.emplace_back(DihedralAtoms {atoms[n], atoms[n + 1], atoms[n + 2], atoms[n + 3]});
            }
            return dihedrals;
        }
    }
    ...
    std::vector<DihedralAtoms> findRotatableDihedralsConnectingResidues(const ResidueLink& link)
    {
        ...
        std::vector<Atom*> firstResidueCyclePoints = FindCyclePoints(link.atoms.first, link.residues.first);
        //    std::cout << "Moving onto second residue.\n";
        std::vector<Atom*> secondResidueCyclePoints = FindCyclePoints(link.atoms.second, link.residues.second);
        ...
        std::vector<DihedralAtoms>& rotatableDihedralsInBranches = std::get<0>(branchResult);
        std::vector<Atom*>& connectingAtoms = std::get<1>(branchResult);
        std::vector<DihedralAtoms> RotatableDihedrals = splitAtomVectorIntoRotatableDihedrals(connectingAtoms);
