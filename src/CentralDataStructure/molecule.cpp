...
namespace gmml
{
    ...
    // Move Ctor
    Molecule::Molecule(Molecule&& other) noexcept : glygraph::Node<Molecule>(other)
    {
    ...
    // Copy Ctor
    Molecule::Molecule(const Molecule& other) : glygraph::Node<Molecule>(other), number_(other.number_)
    {
    ...
    {
        glygraph::Node<Molecule>::operator=(other); // ToDo ok?
        swap(*this, other);
        return *this;
    }
    ...
    std::vector<Atom*> Molecule::getAtoms() const
    {
        ...
        {
            util::insertInto(atoms, residue->getAtoms());
        }
        return atoms;
    }
    ...
    std::vector<Atom*> Molecule::mutableAtoms()
    {
        ...
        for (auto& residue : residues_)
        {
            util::insertInto(atoms, residue->mutableAtoms());
        }
        return atoms;
    }
    ...
    std::vector<Residue*> Molecule::getResidues(const std::vector<std::string>& queryNames)
    {
        ...
        std::function<bool(const size_t&)> hasName = [&](size_t n) { return util::contains(queryNames, names[n]); };
        return util::indicesToValues(residues, util::vectorFilter(hasName, util::indexVector(residues.size())));
    }
