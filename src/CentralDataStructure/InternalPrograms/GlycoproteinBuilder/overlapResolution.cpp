...
namespace glycoproteinBuilder
{
        ...
        int numThreads)
    {
        ...
        MetadataOrder randomMetadataOrder = [](pcg32& rng,
                                               const GlycamMetadata::DihedralAngleDataTable& table,
        ...
        auto randomAngle =
            [&standardDeviation](
                pcg32& rng, const AngleSettings& settings, const GlycamMetadata::DihedralAngleData& metadata)
        {
                                                   ...
                                                   size_t linkageId)
        {
            return randomLinkageShapePreference(
                rng, settings, data, mutableData.bounds, linkageId, randomAngle, freezeGlycositeResidueConformation);
        };
        ...
        SidechainAdjustment adjustSidechains = [&](pcg32& rng,
                                                   ...
                                                   const std::vector<cds::GlycanShapePreference>& glycositePreferences,
                                  ...
                                  size_t glycanId)
        {
            auto searchAngles =
                [&standardDeviation,
                 &settings](const GlycamMetadata::DihedralAngleData& metadata, double preference, double deviation)
            {
                auto std = standardDeviation(settings, metadata);
                return cds::evenlySpacedAngles(
                    preference, deviation * std.first, deviation * std.second, settings.searchIncrement);
            };
            return wiggleGlycan(
                graph, data, selection, {settings.searchDeviation, searchAngles}, preference, mutableData, glycanId);
        };
            ...
            codeUtils::writeToFile(
                ...
                [&](std::ostream& stream)
                {
                    cds::WriteResiduesTogetherToOffFile(
                        stream, graph, offData, codeUtils::boolsToIndices(residueIncluded), "GLYCOPROTEINBUILDER");
                });
        };
        ...
        auto writePdbFile = [&](const assembly::Graph& graph,
                                ...
                                const std::string& prefix)
        {
            cds::PdbFileData pdbData = toPdbFileData(
            ...
            codeUtils::writeToFile(
                ...
                [&](std::ostream& stream)
                {
                    cds::writeAssemblyToPdb(stream, graph, residueIndices, TER, connectionIndices, pdbData);
                    cds::theEnd(stream);
                });
        };
...
/*    */#pragma omp parallel for
        ...
        codeUtils::writeToFile(
            outputDir + "/summary.txt", [&](std::ostream& stream) { codeUtils::toTxt(stream, textStructure); });
        codeUtils::writeToFile(
            outputDir + "/summary.html", [&](std::ostream& stream) { codeUtils::toHtml(stream, textStructure); });
        codeUtils::writeToFile(
            outputDir + "/structures.csv",
            [&](std::ostream& stream) { codeUtils::toCsv(stream, ",", summary.structuretable); });
    }
} // namespace glycoproteinBuilder
