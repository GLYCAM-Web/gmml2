...
namespace gmml
{
    namespace
    {
        ...
        Sphere boundingSphereInitialEstimate(const std::vector<Sphere>& spheres)
        {
            ...
            for (size_t n = 0; n < 3; n++)
            {
                ...
                double span = distance(center(minp), center(maxp)) + radius(minp) + radius(maxp);
                if (span > maxSpan)
                {
                    maxSpan = span;
                    maxSpanN = n;
            ...
            Coordinate finalCenter = scaleBy(0.5, center(spheres[minId[maxSpanN]]) + center(spheres[maxId[maxSpanN]]));
        ...
        Sphere boundingSphereIncludingAllSpheres(Sphere sphere, const std::vector<Sphere>& spheres)
        {
            for (auto& a : spheres)
            {
                sphere = boundingSphereIncluding(sphere, a);
            }
            return sphere;
        }
    } // namespace
    ...
    Sphere boundingSphereIncluding(Sphere sphere, const Sphere& include)
    {
        ...
        if (dist > sphere.radius)
        {
            ...
            Coordinate newCenter = sphere.center + scaleBy((dist - sphere.radius) / (2.0 * diffLength), diff);
