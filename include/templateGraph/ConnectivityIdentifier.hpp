#ifndef TEMPLATEGRAPH_CONNECTIVITYIDENTIFIER_HPP
#define TEMPLATEGRAPH_CONNECTIVITYIDENTIFIER_HPP
        ...
        for (glygraph::Node<T>* currNeigh : nextNode_t->getNeighbors())
        {
            ...
            if (preTime_t[currNeighIndex] == -1)
            {
                ...
                if (lowestTime_t[currNeighIndex] == preTime_t[currNeighIndex])
                {
                    ...
                    glygraph::Edge<T>* connectingEdge = currNeigh->getConnectingEdge(nextNode_t->getDerivedClass());
