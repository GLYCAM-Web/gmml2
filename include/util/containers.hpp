#ifndef INCLUDE_UTIL_CONTAINERS_HPP
#define INCLUDE_UTIL_CONTAINERS_HPP
...
namespace gmml
{
    namespace util
    {
        ...
        template<class T>
        std::vector<T> vectorFilter(std::function<bool(const T&)>& condition, const std::vector<T>& vec)
        {
            std::vector<T> result;
            result.reserve(vec.size());
