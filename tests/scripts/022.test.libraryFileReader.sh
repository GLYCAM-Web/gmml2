...
g++ -std=c++17 -I "${GMML_ROOT_DIR}"/ -L"${GMML_ROOT_DIR}"/lib/ -Wl,-rpath,"${GMML_ROOT_DIR}"/lib/ src/022.test.cdsLibFile.cpp -lgmml2 -pthread -o libFileReader
./libFileReader >022.output.txt

fileList=("libAsPdbFile.pdb" "libAsOffFile.off" "libAsLibFile.lib")
for file in "${fileList[@]}"; do
    if [ -f "${file}" ]; then
        if ! cmp "${file}" correct_outputs/022."${file}" >/dev/null 2>&1; then
            echo -e "Test FAILED!\n ${file} is different from correct_outputs/022.${file}\n"
            echo "Exit Code: 1"
            return 1
        else
            rm "${file}"
