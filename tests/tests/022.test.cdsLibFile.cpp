...
int main()
{
    ...
    try
    {
        codeUtils::writeToFile(fileName, [&](std::ostream& stream) { cds::WritePdb(stream, moleculeIndices, {}); });
    }
    catch (...)
    {
    ...
    try
    {
        codeUtils::writeToFile(
            fileName, [&](std::ostream& stream) { cds::WriteOff(stream, "MOLECULE", moleculeIndices); });
    }
    catch (...)
    {
